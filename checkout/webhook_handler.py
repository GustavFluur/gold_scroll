...
class StripeWH_Handler:
    ...
    def purchase_handler_succeeded(self, event):
        """
        Handle the payment_intent.succeeded webhook from Stripe
        """
        intent = event.data.object
        pid = intent.id
        acquisition = intent.metadata.bag
        ...
        while attempt <= 5:
            try:
                order = Order.objects.get(
                    ...
                    zip_code__iexact=shipping_details.address.zip_code,
    ...
    def purchase_handler_failed(self, event):
        """
